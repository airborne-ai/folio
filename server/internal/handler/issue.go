...
import (
	...
	"github.com/airborne23/folio/server/internal/logger"
	"github.com/airborne23/folio/server/internal/util"
	"github.com/airborne23/folio/server/pkg/agent"
	db "github.com/airborne23/folio/server/pkg/db/generated"
	"github.com/airborne23/folio/server/pkg/protocol"
)
...
// into a `folio issue create` invocation in the background; success and
...
func (h *Handler) QuickCreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	// Daemon CLI version gate removed (per operator request 2026-05).
	// Quick-create now proceeds regardless of the daemon's reported CLI
	// version. The helper checkQuickCreateDaemonVersion is kept (with
	// its tests) in case the gate needs to be re-enabled later.
...
// metadata column is JSONB on the wire; the daemon stores the Folio CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See folio-ai/folio#1660.
